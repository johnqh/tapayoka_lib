...
interface VendorInstallationsState {
  ...
  setInstallations: (installations: VendorInstallation[], parentId: string, entitySlug?: string) => void;
  addInstallation: (installation: VendorInstallation) => void;
  updateInstallation: (id: string, updates: Partial<VendorInstallation>) => void;
  removeInstallation: (id: string) => void;
  reset: () => void;
}

export const useVendorInstallationsStore = create<VendorInstallationsState>(set => ({
  installations: [],
  isLoaded: false,
  entitySlug: null,
  parentId: null,
  setInstallations: (installations, parentId, entitySlug) =>
    set({ installations, isLoaded: true, parentId, entitySlug: entitySlug ?? null }),
  ...
  reset: () => set({ installations: [], isLoaded: false, entitySlug: null, parentId: null }),
}));
